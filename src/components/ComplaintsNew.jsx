    ...
    <Container className="bg-light rounded-4 ps-2 ">
          ...
          {complaints.map((item, index) => (
            ...
            >
              <Row className=" d-flex align-items-center justify-content-around w-100' ">
                <Col md={1}>
                  <Image
                    src={`https://cdn.auth0.com/avatars/${formatToInitials(
                      item.userName
                    )}.png`} // İlk harflerden oluşan URL
                    roundedCircle
                    width={50}
                    className="me-3"
                  />
                ...
                <Col md={4} className="ms-2">
                  <div>
                    ...
                    <Card.Text className="text-start">{item.type}</Card.Text>
                  </div>
                </Col>
                <Col md={2}>
                  <h3>{item.roomName}</h3>
      ...
      <Modal show={show} onHide={handleClose} centered>
        ...
        <Modal.Footer>
          ...
          <Button onClick={handleResolveComplaint}>Şikayeti çöz</Button>
      ...
      <Modal show={showResolveModal} onHide={handleResolveClose} centered>
        ...
        <Modal.Footer>
          ...
          <Button onClick={handleSubmitResponse}>Geri Dönüşü Gönder</Button>
        </Modal.Footer>
      </Modal>
    </Container>
  );
